...
const RAGDOLL_PARTS: [RagdollPartSpec; 12] = [
    RagdollPartSpec {
        bone_name: "root.x",
    ...
    RagdollPartSpec {
        bone_name: "hand.l",
        shape: RagdollShape::Sphere { radius: 0.09 },
    },
    RagdollPartSpec {
...
const RAGDOLL_JOINTS: [(&str, &str); 11] = [
    ("root.x", "spine_01.x"),
    ...
    ("thigh_stretch.l", "foot.l"),
    ("root.x", "thigh_stretch.r"),
    ("thigh_stretch.r", "foot.r"),
];
...
fn main() {
    App::new()
        ...
        .add_systems(Update, (switch_animation, toggle_ragdoll))
        .add_systems(
            PostUpdate,
...
fn setup(
    ...
    info!("Controls: 1/2 switch animation, Enter cycles animation, R toggles ragdoll physics");
}
...
fn spawn_ragdoll(
    ...
    for (parent_name, child_name) in RAGDOLL_JOINTS {
        ...
        // Free spherical joints: a near-rigid point constraint (tiny compliance
        // solves far more stably than 0.0) with NO angular limits. Avian's
        // swing/twist limits need a per-joint basis aligned to each bone's rest
        // orientation; without it the rest pose reads as fully violated and the
        // limit correction injects huge angular velocity. Angular motion is
        // instead tamed by `AngularDamping`.
        commands.spawn((
            RagdollJoint,
